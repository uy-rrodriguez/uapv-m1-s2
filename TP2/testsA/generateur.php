...
class GenerateurUnitTestCase {
    /**
     * Constructeur qui définit les listes à utiliser
     */
    ...
    /**
     * Écriture des entêtes dans le fichier de sortie.
     * On va définir le rapporteur et la classe à tester.
     */
    private function ecrireEntete($fout, $pathSimpleTest, $pathRapporteur, $pathClasseTester) {
        echo "<div>ecrireEntete</div>" . PHP_EOL;

        $contenu = file_get_contents("template/head.txt");
        $contenu = str_replace("[!SIMPLE_TEST!]", $pathSimpleTest, $contenu);
    ...
    /**
     * Écriture du pied.
     * Ici on va ajouter tout ce qui doit apparaître à la fin du fichier.
     * Pour l'instante on ferme juste le code de la classe de test.
     */
    ...
    /**
     * Fonction pour écrire chaque test.
     * Tous les tests ont un nom, une liste de valeurs à envoyer à la fonction à tester
     * et une valeur pour le résultat attendu.
     */
    private function ecrireTest($fout, $nomTest, $valeurs, $resultatAttendu) {
        echo "</div>ecrireTest $nomTest</div><br />" . PHP_EOL;
    ...
    /**
     * On prends en paramètre un fichier d'entrée $data et un autre de sortie $out.
     * On va parcourir les données, les traiter et finalement écrire les différents
     * tests dans le fichier PHP de sortie.
     */
    function generer($data, $out) {
        ...
        // Entete
        $this->ecrireEntete($fout,
                            dirname(__FILE__) . "/../simpletest/autorun.php",
                            dirname(__FILE__) . "/MonRapporteurHTML.class.php",
                            dirname(__FILE__) . "/queFaireAujourdhui.php");
