...
public class Emetteur {
	...
	private void run() throws Exception {
		...
		try {
			...
			jms.creerConnexion(null);
			//jms.creerSession();
			System.out.println("Connection créée (inactive)");
			...
			// Envoi d'un message qui va expirer plus tot
			TextMessage msg = jms.getSession().createTextMessage();
			msg.setText("MESSAGE QUI EXPIRE (pour " + destinataire + ")");
			...
			// Expiration
			sender.setTimeToLive(500);
			
			// Envoi du message
			sender.send(msg);
			
			System.out.println("Message QUI EXPIRE envoyé à " + destinataire + "!");
			System.out.println();
		}
		catch(Exception e) {
			throw e;
